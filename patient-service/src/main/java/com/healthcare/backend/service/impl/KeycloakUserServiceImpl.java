...
public class KeycloakUserServiceImpl implements KeycloakUserService {
    ...
    @Value("${keycloak.role.patient}")
    public String keycloakPatientRole;
        ...
        try (Response response = usersResource.create(user)) {
            if (response.getStatus() == 201) {
                ...
                RoleRepresentation patientRole = keycloak.realm(realm).clients().get(keycloakClientId).roles()
                        .get(keycloakPatientRole).toRepresentation();
                ...
                log.info("Created user: " + user.getUsername() + ", id: " + createdUser.getId() + "!");
